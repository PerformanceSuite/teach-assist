...
const STATUS_CONFIG = {
  ready_for_review: {
    label: 'Ready',
    color: 'text-yellow-600 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-400/10',
    icon: Clock,
  },
  approved: {
    label: 'Approved',
    color: 'text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-400/10',
    icon: CheckCircle2,
  },
  needs_attention: {
    label: 'Needs Attention',
    color: 'text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-400/10',
    icon: AlertTriangle,
  },
  error: {
    label: 'Error',
      ...
      {isEditing ? (
        <div className="space-y-3">
          <textarea
            ...
            rows={6}
            className="w-full bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-3 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
          />
        ...
        <>
          ...
          {narrative.structure.achievement ? (
            <div className="space-y-2">
              <StructuredSentence label="Achievement" text={narrative.structure.achievement} color="text-blue-600 dark:text-blue-400" />
              <StructuredSentence label="Evidence" text={narrative.structure.evidence} color="text-emerald-600 dark:text-emerald-400" />
              <StructuredSentence label="Growth" text={narrative.structure.growth} color="text-amber-600 dark:text-amber-400" />
              <StructuredSentence label="Outlook" text={narrative.structure.outlook} color="text-purple-600 dark:text-purple-400" />
            </div>
          ) : (
          ...
          {Object.keys(narrative.council_review).length > 0 && (
            <div className="mt-3 pt-3 border-t border-gray-300 dark:border-gray-700">
              <div className="text-xs text-gray-500 dark:text-gray-500 mb-2">Council Review</div>
            ...
            {narrative.status !== 'approved' && (
              <>
                <button
                  onClick={handleFlag}
                  disabled={isSaving}
                  className="flex items-center gap-1 px-3 py-1.5 bg-yellow-100 dark:bg-yellow-600/20 hover:bg-yellow-200 dark:hover:bg-yellow-600/30 text-yellow-600 dark:text-yellow-400 rounded-lg text-sm font-medium transition-colors"
                >
...
function StructuredSentence({ label, text, color }: { label: string; text: string; color: string }) {
  ...
  return (
    <div className="flex gap-3 items-start">
      ...
      <p className="text-gray-700 dark:text-gray-200 text-sm leading-relaxed">{text}</p>
    </div>
  )
}
